...
Integrating state_machine into your Rails 3.0+ application is straightforward. 
...
In your Gemfile, add the lines:
  ...
  gem 'state_machine'
  gem 'ruby-graphviz', :require => 'graphviz'  # only if you want graphing, see above

As usual, run +bundle+ +install+ to load the gems.

=== 2. Create a model

Create a model with at least a +state+ field, along with other any
fields your application requires:

  $ rails generate model Vehicle state:string seatbelt_on:boolean time_used:datetime
  $ rake db:migrate

=== 3. Configure the state machine

Add the state machine to your model.  Following the example above,
